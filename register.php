...
if ($accCheckResult < 1) {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
      $sql = "INSERT INTO accounts (username, password, privilege) VALUES (?, ?, 'user')";
        ...
        <div class="card shadow-2-strong border border-dark" style="border-radius: 1rem;">
          <div class="shadow-lg card-body p-5 text-center" style="border-radius: 1rem;">
            ...
            <p>
              <?php
                if ($accCheckResult < 1) {
                  echo 'Please fill this form to create an ADMIN account.';
                } else {
                  echo 'Please fill this form to create a USER account.';
                }
              ?>
            </p>
            <form autocomplete="off" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
              ...
              <div class="form-group">
                <?php
                if ($accCheckResult < 1) {
                  echo '<br><center><input type="submit" class="btn btn-outline-primary shadow btn-block" style="width: 60%;" value="Register"></center>';
                } else {
